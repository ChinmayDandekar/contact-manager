...
export const generateSpamReport = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(!phoneExists) throw  new NotFoundException("Phone Number Not In Global Database", ErrorCode.PHONE_NOT_FOUND)
    ...
    if(spamReport) throw new BadRequestsException("Report Already Exists", ErrorCode.REPORT_ALREADY_EXISTS)
